...
def chronological_predictions(games: list[Game], model: EloModel | None = None):
    ...
    The normalized contract guarantees a date, not a reliable tip-off timestamp.
    Every game on one date is therefore predicted from the same entering-day
    ratings. That date's results are applied only after all predictions are saved.
    """
