...
def test_leverage_preserves_other_forced_results_inside_candidate_branches():
    games = synthetic_demo_games()
    as_of = date(2026, 1, 15)
    future = [g for g in games if g.game_date >= as_of]
    fixed, candidate = future[0], future[1]
    baseline = evaluate_game_leverage(games, candidate, as_of, trials=250, seed=33)
    altered = evaluate_game_leverage(
        games,
        candidate,
        as_of,
        trials=250,
    ...
    assert altered.title_distribution_shift >= 0
    assert (
        abs(altered.title_distribution_shift - baseline.title_distribution_shift) > 1e-9
        or abs(altered.playoff_distribution_shift - baseline.playoff_distribution_shift) > 1e-9
    )
